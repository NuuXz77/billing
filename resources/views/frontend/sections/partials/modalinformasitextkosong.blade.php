...
<!-- Info Modal -->
<div id="infoModal" class="hidden fixed inset-0 bg-gradient-to-br from-black/40 to-black/60 flex items-center justify-center z-[99999] modal-backdrop-fade backdrop-blur-sm" onclick="if(event.target === this) { closeInfoModal(); }">
  <div class="bg-white rounded-2xl shadow-2xl max-w-sm mx-4 modal-slideup overflow-hidden" onclick="event.stopPropagation();">
    ...
    <div class="px-8 py-4 bg-white border-t border-gray-100 flex gap-3">
      <button 
        type="button" 
        class="flex-1 bg-blue-600 text-white px-6 py-2.5 rounded-lg text-sm font-semibold cursor-pointer transition-all duration-300 hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 active:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2" 
