...
<div id="viewDetailsModal-{{ $transaction->id }}" class="hidden fixed inset-0 bg-gradient-to-br from-black/40 to-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 modal-backdrop-fade" onclick="if(event.target === this) closeModal('viewDetailsModal-{{ $transaction->id }}')">
    <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col modal-slideup" onclick="event.stopPropagation();">
        ...
        <div class="p-6 overflow-y-auto flex-1 space-y-5">
            ...
            @if(($transaction->subdomain_web || $transaction->subdomain_server) || $transaction->admin_notes || ($transaction->payment_proof && in_array($transaction->status, ['pending_confirm', 'active'])))
            <div class="grid grid-cols-1 gap-4 mt-5">
                {{-- Payment Proof Section --}}
                @if($transaction->payment_proof && in_array($transaction->status, ['pending_confirm', 'active']))
                <div class="bg-green-50 border border-green-200 rounded-xl p-4">
                    ...
                    <div class="mt-3 flex justify-center">
                        @if(file_exists(public_path('storage/' . $transaction->payment_proof)))
                        <img src="{{ asset('storage/' . $transaction->payment_proof) }}" 
                             alt="Bukti Pembayaran" 
                             class="w-48 h-auto rounded-lg border-2 border-green-300 shadow-md hover:shadow-xl transition-shadow cursor-pointer object-contain"
                    ...
                    <p class="text-xs text-gray-600 text-center mt-2">Klik gambar untuk memperbesar</p>
                </div>
                @endif
